(async function () {
    ...
    chrome.runtime.onMessage.addListener(function (message) {
        ...
        if (message.canvasInfoList) {
            ...
            document.getElementById('slide-count').innerText = `Slides detected: ${canvasInfoList.length}`;
        }
    });

    async function updateSlideCount() {
        let tabs = await chrome.tabs.query({ active: true, currentWindow: true });
        console.log("Sending message to content script to get canvas info list");
        await chrome.tabs.sendMessage(tabs[0].id, { command: COMMANDS.GET_CANVAS_INFO_LIST }, (response) => {
    ...
    async function getCanvasContent(data) {
        ...
        return new Promise((resolve, reject) => {
            chrome.tabs.sendMessage(tabs[0].id, { command: COMMANDS.GET_CANVAS_DATA, data }, (response) => {
